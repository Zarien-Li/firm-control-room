...
公开版默认是 **operations-only**：`FIRM_CONTINUITY_ENABLED=false`。此时 FIRM
只观察会话、核验任务与资源、投递白名单内的运维事实；它不会调用 Codex、回答停下的
输入框、判断项目价值、选择研究路线或审稿。外部定时 PI/审查任务可以使用 FIRM 提供的
事实，但它始终是独立进程，并拥有自己的策略与审计记录。

启用 Continuity PI 只会打开下文定义的窄范围停点接续，不会把科学组合审查权交给
FIRM，也不能把 FIRM 变成第二审稿人。包括“Codex 只做稀疏后期红队验证”在内的科研
策略属于研究 skills 与主 PI，而不属于控制台。
